...
The R code below reproduces the analyses reported in Hardwicke et al. (2024) "Prevalence of transparent research practices in psychology: A cross-sectional study of empirical articles published in 2022".
...
# select columns were interested in and put them in a useful order.
d_wide <- d_wide %>% 
...
# disagreements were resolved manually at this stage (i.e., by directly adding information to the csv file).

# load in dataframe with disagreements resolved manually, 
d <- read_csv(here('data','processed','d_with_disagreements_resolved.csv'))
```
...
```{r}
# select only agreed upon coding to use in the main analyses
d <- d %>% 
...
This is Table 2 of the manuscript.
...
This is Table 3 of the manuscript.
...
This is Table 4 of the manuscript.
...
```{r}
# Create functional availability variables
contingent_data_analysis <- d_eligible %>% 
  mutate(
    prereg_func = ifelse(prereg_statement == "YES the statement says that there is a preregistration" & prereg_access_status == "YES", T, F),
    materials_func = ifelse(materials_statement == "YES the statement says that the materials are available" & materials_access_status == "YES", T, F),
    data_func = ifelse(data_statement == "YES the statement says that the data are available" & data_access_status == "YES", T, F),
    analysis_func = ifelse(analysis_statement == "YES the statement says that the analysis scripts are available" & analysis_access_status == "YES", T, F),
    data_and_analysis = ifelse(data_func == T & analysis_func == T, T, F)
         ) %>%
  count_prop_ci(data_and_analysis, ci_method = "wilson")

```

As noted by a reviewer, sharing either data or analysis scripts can have independent benefits, but sharing both data and analysis scripts will be maximally beneficial because it may enable computational reproducibility (Hardwicke et al., 2018). Both data and analysis scripts were shared in `r contingent_data_analysis %>% filter(sample_id == "Field", data_and_analysis == T) %>% pull(report)` articles in the field-wide sample and `r contingent_data_analysis %>% filter(sample_id == "Prominent", data_and_analysis == T) %>% pull(report)` articles in the prominent journals sample. 
...
This is Table 5 of the manuscript.
...
This is Table 6 of the manuscript.
...
A comparison of confidence intervals calculated using the Wilson method and the Sison-Glaz method.
...
ci_coi <- bind_rows(
  ...
  mutate(percent = signif(percent,2), ci_lwr = signif(ci_lwr,2), ci_upr = signif(ci_upr,2)) %>%
  arrange(sample, statement, method) %>%
  mutate(indicator = "coi")


# compute the differences between the CIs calculated with the different methods.
compare_ci <- bind_rows(ci_prereg, ci_materials, ci_data, ci_analysis, ci_funding, ci_coi) %>%
  pivot_wider(id_cols = c("indicator", "sample", "statement"),  names_from = "method", values_from = c("percent", "ci_lwr", "ci_upr")) %>%
  mutate(ci_lwr_diff = ci_lwr_SisonGlaz - ci_lwr_Wilson, ci_upr_diff = ci_upr_SisonGlaz - ci_upr_Wilson)
```
The two methods produce highly similar confidence intervals — the largest difference was `r max(c(abs(compare_ci$ci_lwr_diff)),max(abs(compare_ci$ci_upr_diff)))`.
